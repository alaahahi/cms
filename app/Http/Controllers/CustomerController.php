...
class CustomerController extends Controller
{
    ...
    public function edit_clients(Request $request,$id)
    {
        ...
        if($id == 0 ){
            ...
            DB::table('card_user')->insert(array('card_id' => DB::table('cards')->insertGetId(array('card_number' => $item['card_number'],'card_type_id' => $item['card_type_id'],'author_id'=>$userId->id,'created_at'=>$date)),'client_id' =>DB::table('client')->insertGetId(array('full_name' => $item['full_name'],'phone' => $item['phone'],'address'=>$item['address'],'birth_date'=>$item['birth_date'],'author_id'=>$userId->id,'created_at'=>$date)),'strat_active' => $item['strat_active'],'names' =>$item['names'],'end_active' =>  $end_active,'created_at'=>$date,'enter_id'=>$userId->id));
